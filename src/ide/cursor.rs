...
use crate::model::{CursorConfig, InstalledServer, ServerConfig};
...
impl CursorAdapter {
    ...
    fn read_config(&self) -> Result<CursorConfig> {
        if !self.config_path.exists() {
            return Ok(CursorConfig::default());
        }
        let content = std::fs::read_to_string(&self.config_path)?;
        let config: CursorConfig = serde_json::from_str(&content)?;
        Ok(config)
    }

    fn write_config(&self, config: &CursorConfig) -> Result<()> {
...
impl IdeAdapter for CursorAdapter {
    ...
    fn is_detected(&self) -> bool {
        // Cursor is detected if config exists or .cursor directory exists
